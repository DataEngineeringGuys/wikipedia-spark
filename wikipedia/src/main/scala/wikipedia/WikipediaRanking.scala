...
object WikipediaRanking {
  ...
  val conf: SparkConf = new SparkConf().setAppName("Simple Application").setMaster("local[4]")
  ...
  def makeIndex(langs: List[String], rdd: RDD[WikipediaArticle]): RDD[(String, Iterable[WikipediaArticle])] = {
    rdd.flatMap(article => findLanguages(langs, article).map(lang => (lang, article))).groupByKey
  }
   ...
   *   several seconds.
   */
  def rankLangsUsingIndex(index: RDD[(String, Iterable[WikipediaArticle])]): List[(String, Int)] = index.mapValues(item => item.size).collect.toList.sortBy(_._2).reverse
